...
(defparameter *maximaFronteraBusqueda* 0)
  ...
  (let ((nodo '()))
    (cond ((equal metodo :wrong-pieces)
	  ...
	  ((equal metodo :moves-left)
	   (setq nodo (create-node estado operador (count-moves estado *meta*)))
	   (push nodo *open*)
	   (order-open))
	  ...
	  ((equal metodo :custom-value)
	   (setq nodo (create-node estado operador (/ (+(count-moves estado *meta*) (count-wrong-pieces estado *meta*)) 2)))
	   (push nodo *open*)
	   (order-open))
	  (T Nil))))
...
(defun get-max-in-open ()
  (if (> (length *open*) *maximaFronteraBusqueda*)
      (setq *maximaFronteraBusqueda* (length *open*))))
...
(defun aux-manhattan-distance (meta elemento casilla)
  "Funcion que calcula la distancia de cada elemento basandose en la casilla (columna)"
  (let ((contador 0))
    (cond ((= casilla 2)
	   (cond ((equal elemento (third meta)) contador)
		 ((equal elemento (second meta)) (1+ contador))
		 (T (+ 2 contador))))
	  ((= casilla 1)
	   (cond ((equal elemento (third meta)) (1+ contador))
		 ((equal elemento (second meta)) contador)
		 (T (1+ contador))))
	  (T
	   (cond ((equal elemento (third meta)) (+ 2 contador))
		 ((equal elemento (second meta)) (1+ contador))
		 (T contador))))))

(defun manhattan-distance (elemento meta fila casilla)
  "Funcion que calcula la distancia de cada elemento en las filas"
  (cond ((= fila 0)
	 (cond ((member elemento (first meta))
		(+ 0 (aux-manhattan-distance (first meta) elemento casilla)))
	       ((member elemento (second meta))
		(1+ (aux-manhattan-distance (second meta) elemento casilla)))
	       (T (+ 2 (aux-manhattan-distance (third meta) elemento casilla)))))
	((= fila 1)
	 (cond ((member elemento (first meta))
		(1+ (aux-manhattan-distance (first meta) elemento casilla)))
	       ((member elemento (second meta))
		(+ 0 (aux-manhattan-distance (first meta) elemento casilla)))
	       (T (1+ (aux-manhattan-distance (third meta) elemento casilla)))))
	(T
	 (cond ((member elemento (first meta))
		(+ 2 (aux-manhattan-distance (first meta) elemento casilla)))
	       ((member elemento (second meta))
		(1+ (aux-manhattan-distance (second meta) elemento casilla)))
	       (T (+ 0 (aux-manhattan-distance (third meta) elemento casilla)))))))

(defun count-moves (estado meta)
  "Funcion que nos permite calcular la distancia Manhattan del estado actual"
  (let ((mov-cada-ficha 0)
	(columna 0)
	(fila 0)
	(casilla 0)
	(elementos (smooth estado)))
    (loop for elem in elementos do
	 (if (= columna 3)
	     (progn
	       (setq casilla 0)
	       (setq fila (1+ fila))
	       (setq columna 0)))
	 (setq mov-cada-ficha (+ mov-cada-ficha (manhattan-distance elem meta fila casilla)))
	 (setq casilla (1+ casilla))
	 (setq columna (1+ columna)))
    mov-cada-ficha))
...
(defun display-solution (lista-nodos)
  ...
  (format  t "Longitud maxima de frontera de busqueda ~A ~%" *maximaFronteraBusqueda*)
...
;(informed-search '((2 8 3)(1 4 5)(7 x 6)) '((1 2 3)(8 x 4)(7 6 5)) :moves-left)
;(informed-search '((2 8 3)(1 4 5)(7 x 6)) '((1 2 3)(8 x 4)(7 6 5)) :random-value)
;(informed-search '((2 8 3)(1 4 5)(7 x 6)) '((1 2 3)(8 x 4)(7 6 5)) :custom-value)
